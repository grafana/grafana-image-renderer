		...
		Flags: slices.Concat([]cli.Flag{
			...
			&cli.StringSliceFlag{
				...
				Sources: config.FromConfig("auth.token",
					config.FromEnv("GF_RENDERER_AUTH_TOKEN")),
			},
			...
			&cli.StringFlag{
				...
				Sources: config.FromConfig("browser.path",
					config.FromEnv("GF_RENDERER_BROWSER_PATH")),
			},
			&cli.StringSliceFlag{
				Name:     "browser-flags",
				...
				Sources: config.FromConfig("browser.flags",
					config.FromEnv("GF_RENDERER_BROWSER_FLAGS")),
			},
			&cli.BoolFlag{
				Name:     "browser-gpu",
				...
				Sources: config.FromConfig("browser.gpu",
					config.FromEnv("GF_RENDERER_BROWSER_GPU")),
			},
		}, traces.TracerFlags()),
		Action: run,
	}
