...
func TestRenderingGrafana(t *testing.T) {
	...
	t.Run("render PDF with no data", func(t *testing.T) {
		t.Parallel()
		...
		t.Run("returns the PDF with no data in the panel", func(t *testing.T) {
			t.Parallel()
			...
			req.Header.Set("X-Auth-Token", "-")
			query := req.URL.Query()
			query.Set("url", "http://grafana:3000/d/no-data-prom-testing?render=1&from=1699333200000&to=1699344000000&kiosk=true")
			query.Set("encoding", "pdf")
			query.Set("renderKey", renderKey)
			query.Set("domain", "grafana")
			req.URL.RawQuery = query.Encode()
		...
		t.Run("returns an empty response but no PDF", func(t *testing.T) {
			t.Parallel()
			...
			query.Set("renderKey", renderKey)
			query.Set("domain", "grafana")
			query.Set("emptyResponseOnNoQueries", "true")
			req.URL.RawQuery = query.Encode()

			resp, err := http.DefaultClient.Do(req)
			require.NoError(t, err, "could not send HTTP request to Grafana")
			require.Equal(t, http.StatusNoContent, resp.StatusCode, "unexpected HTTP status code from Grafana")
		})
	})
}
